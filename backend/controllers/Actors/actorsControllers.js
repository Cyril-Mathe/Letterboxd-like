...
export async function getActorsById(req, res) {
    try {
...
export async function createActors(req, res) {
    try {
        const {
            name,
            birth_date,
        ...
        const result = await pool.query(
            `INSERT INTO actors (name, birth_date, death_date, place_of_birth, nationnality, img_url) VALUES ($1, $2, $3, $4, $5, $6) RETURNING *`,
...
export async function modifyActors(req, res) {
    try {
        const id = req.params.id;
        const {
            name,
        ...
        const result = await pool.query(`UPDATE actors SET name = $1, birth_date = $2, death_date = $3, place_of_birth = $4, nationnality = $5, img_url = $6 WHERE id = $7 RETURNING *`, [name, birth_date, death_date, place_of_birth, nationality, img_url, id]);
...
export async function deleteActors(req, res) {
    try {
        const id = req.params.id
        const result = await pool.query('DELETE FROM actors WHERE id = $1 RETURNING *', [id]);
        res.status(200).json({ message: 'Acteur supprimé', data: result.rows[0] });
    } catch (error) {
        res.status(500).json({ message : error.message})
    }
}